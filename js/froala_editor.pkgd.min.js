...
window.loadAllImages = function (event) {
	debugger
	$.ajax({
		headers: {
			"Accept": "application/json",
			"Content-type": "application/x-www-form-urlencoded"
		}, url: "api/api/services/app/MediaLibrary/GetMediaLibrariesFroalaFormat?MaxResultCount=1000000",
		success: function (response) {
			$('#fr-image-list .fr-list-column .fr-image-container').empty();
			$('.fr-list-column').empty();
			$('#fr-modal-tags').empty();
			...
			for (index = 0; index < response.length; index++) {
				$('#fr-image-list .fr-list-column:nth-child(' + column + ')').append(
					'<div class="fr-image-container fr-image-' + index + '" data-loading="Loading.." data-deleting="Deleting.." style="height: auto;"><img src= "' + response[index].url + '" data-url="' + response[index].thumb + '" data-tag="' + response[index].tag + '" alt="' + response[index].name + '" data-id="' + response[index].id + '" data-name="' + response[index].name + '" data-type="' + response[index].type + '"><i class="fa fa-trash fr-delete-img" aria-hidden="true" title="Delete"></i><i class="fa fa-plus fr-insert-img" aria-hidden="true" title="Insert"></i></div>');
				$('#fr-modal-tags').append('<a role="button" onclick = "closeMenu();" title="' + response[index].tag + '" style="opacity: 1;">' + response[index].tag + '</a>')
				if (column == 4) {
					column = 1;
				} else {
					column++;
...
// ** select an image name for insertion on hamburger menu*/
window.closeMenu = function () {
	if (document.getElementsByClassName("fr-modal-head")[0].classList.contains('fr-show-tags')) {
		document.getElementById("fr-modal-tags").setAttribute("style", "display: none;")
		document.getElementById('fr-modal-head').setAttribute("style","height:0px");
	}		
}
window.showTags = function (event) {
	document.getElementById("fr-modal-tags").setAttribute("style", "display: block;")
}
