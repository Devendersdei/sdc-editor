...
(function(){
  ...
  function setCaretToPos (input, pos) {
    setSelectionRange(input, pos, pos);
	 ...
	 // ** Cursor Selection */
        curIndex = 0;
       var el = $('#gss-search').get(0);
       curIndex = el.selectionStart;  
///////////////////////////
       totlen = $('#gss-search').val().length;
      var textComponent = document.getElementById('gss-search');
      var startPos = textComponent.selectionStart;
      var endPos = textComponent.selectionEnd;
      var selectedText = textComponent.value.substring(startPos, endPos).length;
      // ** Function for triggering the event */
        if(event.key == 'Backspace'){
           if(selectedText == totlen ) {
               searchTerm = searchTerm.slice(0, 0);
               $('#gss-search').val(searchTerm)
                ...
                if(selectedText == 0) {
                  if (curIndex == totlen ) {
                    ...
                    // setCaretToPos(document.getElementById("gss-search"), 4);
                    searchTerm =  searchTerm.slice(0, (curIndex-1)) + searchTerm.slice(curIndex);
                    $('#gss-search').val(searchTerm)
                    setCaretToPos($("#gss-search")[0], curIndex-1);
                  }
                 
                 
                } else {
                   searchTerm =  searchTerm.slice(0, startPos) + searchTerm.slice(endPos);
                   $('#gss-search').val(searchTerm)
                }
               }
            ...
            if(selectedText == totlen ) {
                ...
                searchTerm =  searchTerm.slice(0, startPos) + searchTerm.slice(endPos);
                $('#gss-search').val(searchTerm)
                setCaretToPos($("#gss-search")[0], curIndex);
            }
            else {
              searchTerm =  searchTerm.slice(0, (curIndex)) + searchTerm.slice(curIndex+1);
                $('#gss-search').val(searchTerm)
                setCaretToPos($("#gss-search")[0], curIndex);
            }
      }
    		    ...
    		    if(isValid(event)){
              var output =  [searchTerm.slice(0, curIndex), event.key, searchTerm.slice(curIndex)].join('');
               // searchTerm += event.key
               searchTerm = output
            }
            $('#gss-search').val(searchTerm)
            if(curIndex != totlen) {
              if((event.keyCode > 64 && event.keyCode < 91) || (event.keyCode > 47 && event.keyCode < 58) || (event.keyCode > 95 && event.keyCode < 112) || (event.keyCode > 185 && event.keyCode < 193)) {
                setCaretToPos($("#gss-search")[0], (curIndex + 1) );
              }
            }
      }
      ...
      $(".fr-image-container >  img").filter(function() {
        var text = $(this).attr('data-tag').replace(/\s+/g, ' ').toLowerCase();
        return text.includes(searchText) == true;
    }).parent().fadeIn();
      // $(".fr-image-container >  img").filter(function() {
      // //  return $(this).attr('data-tag').toLowerCase().indexOf(searchText) > -1;
      // // }).parent().fadeIn();
      //    return $(this).attr('data-tag:contains(' + searchText + ')').toLowerCase().indexOf(searchText) > -1;
      //   }).parent().fadeIn();
	  	if(searchTerm == ''){
    ...
    // return false;
      ...
      //**End Key */
      if(e.keyCode == 35) {
        setCaretToPos($("#gss-search")[0], (totlen) );
        return false;
      }
      //** Home Key */
      if(e.keyCode == 36) {
        setCaretToPos($("#gss-search")[0], 0 );
      }
  ...
  var htmlEl = document.getElementsByClassName('fr-modal-tags');
  htmlEl.classList.remove('overflow', hidden);
htmlEl.classList.add('overflow', scroll);
})();
